...
SWOMSE is an R package designed to evaluate alternative management policies for the North Atlantic Swordfish fishery. The package is built based on the [DLMtool](https://cran.r-project.org/package=DLMtool) and [MSEtool](https://cran.r-project.org/package=MSEtool) packages, and all functions and features of those packages are available in SWOMSE.

More information on the SWOMSE framework is available in the SWOMSE [user manual](https://iccat.github.io/nswo-mse/userguide.html) and the [Trial Specifications Document](https://docs.google.com/document/d/1L_1P5L3lcikzNhOSCOdUBSDfsa_NwN0tpIWvzxQ5xWw/edit?usp=sharing). All documents relating to the SWOMSE package can be found on the [North Atlantic Swordfish MSE homepage](https://iccat.github.io/nswo-mse/)^[https://iccat.github.io/nswo-mse/]
...
```{r}
Hist@Misc$ErrList$Ind_Stat[1:2,1:4]
```
...
```{r, cache=TRUE}
MSE <- runMSE(OM, MPs="AvC") # run MSE with average catch MP
```
...
SimData <- MSE@Misc$Data[[1]] # extract the simulated data 
...
In this example we use a delay-difference stock assessment model from the `MSEtool` package, linked with a 40-10 harvest control rule:

```{r}
CMP_m1 <- function(x, Data, ...) {
  runAssess <- MSEtool::DD_TMB(x, Data) # run the delay-difference model
  Rec <- MSEtool::HCR_ramp(runAssess, LRP=0.1, TRP=0.4) # 40-10 HCR
  Rec # return Rec object
}
class(CMP_m1) <- 'MP'
```

All assessment methods in the `MSEtool` package are available in the `SWOMSE` framework:
...
Custom assessment methods can be included by replacing the call to `MSEtool::DD_TMB` with a code for a stock assessment model. This can include calls to external stock assessment software (e.g., SS3). 
...
sim <- 1
par(mfrow=c(1,2))
plot(MSE@Misc$Data[[1]]@Ind[sim,68:97], type="l")
abline(h=200, lty=3)
plot(MSE@C[sim,1,], type="l")
