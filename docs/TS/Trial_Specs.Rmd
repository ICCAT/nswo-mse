...
The 2017 assessment did not explicitly model the retention curves and discard mortality.
...
The 216 OMs in the uncertainty grid, conditioned using SS3, were imported into MSE framework using the `SS2OM` function in `MSEtool` (see [here](https://msetool.openmse.com/reference/SS2MOM.html) for details on the function).
...
A SCRS report summarizing the parameters, estimated reference points, and predicted stock dynamics for the 216 OMs in the uncertainty grid was presented at the 2021 intersessional meeting (May 31 - June 4 2021). A copy of the report is available on the North Atlantic Swordfish MSE [homepage](https://iccat.github.io/nswo-mse/).
...
> Note: The OMs were conditioned with data up to 2017, and therefore the projection period for the simulations currently starts from 2018. If catch data from 2018 to 2020 (or 2021) is available, the model can use the actual catches for the first few years of the projections, rather than assume that the cMPs are implemented in 2018.

## Historical Data

Fishery data was generated by the MSE framework at the end of the spool-up period (currently 2017, see note above). There were three main sources of data available to the CMPs: 1) an index of abundance, 2) catch data, 3) length composition data.
...
> Note: the Group needs to discuss the appropriate (if any) degree of implementation error for the TAC recommendations provided by the CMPs.
...
Based on feedback from the Commission's Panel 4, the following performance metrics have been proposed for swordfish:
