...
![(#fig:data-fig) Summary of the data used in the 2022 stock assessment.](../../img/Data_overview.png){width=50%}
...
![(#fig:rec-scen) The Base Case and 3 scenarios for persistent patterns in recruitment deviations in the projection years.](../../img/R7_Recruitment_Scenarios.png){width=50%}
...
![(#fig:catch-plot) The historical total catch and total allowable catch (TAC) for the North Atlantic swordfish.](../../img/Catch_TAC.png){width=50%}
...
![(#fig:index-plot) The Combined Index for the North Atlantic swordfish fishery.](../../img/Combined_Index.png){width=50%}
...
![(#fig:addindex-plot) The additional individual fishery-dependent and survey indices for the North Atlantic swordfish fishery.](../../img/Individual_Indices.png){width=50%}
...
Data lags are then handled in the CMP code, where the default assumption is that the catch and index data are lagged by 2 years. See the CMP (see the [CMP Development Guide](../cMPdevelopment/CMP-Development-Guide.html) for more details).
...
3. **Yield**: the catch in the projection years (ton)
...
`PGK_10` calculates the probability of being in Green Zone of Kobe Space (SB>SBMSY & F<FMSY) in year 10 (2033). This is calculated as the proportion of the total number of simulations that are in the Green Zone of Kobe Space in 2033.

Figure \@ref(fig:PGK10) shows an example of two CMPs (`r PMlist$MPs`) for an operating model with `r MSE_1@nsim` simulations. The values for `PGK_10` in this example are: `r PMlist$PGK_10`.

![(#fig:PGK10) An example of time-series plots of F/FMSY and SB/SBMSY for two CMPs. This OM has 10 simulations, shown as the colored lines. Simulations where SB>SBMSY and F<FMSY in 2033 are indicated with a green point, and those outside of the green space in the Kobe plot are indicated in red. The Kobe plot on the bottom shows these points in the Kobe space. The numbers in the corners indicate the percent of points in each area of the Kobe space.](../../img/PMs/PGK_10.png){width=75%}

### PGK_med

`PGK_med` calculates the probability of being in Green Zone of Kobe Space (SB>SBMSY & F<FMSY) over years 6--10 (2029--2033). This is calculated as the proportion of the total points (number of simulations x number of years) that are in the Green Zone of Kobe Space.
...
![(#fig:PGKmed) An example of time-series plots of F/FMSY and SB/SBMSY for two CMPs. This OM has 10 simulations, shown as the colored lines. Simulations where SB>SBMSY and F<FMSY in years 2029--2033 are indicated with a green point, and those outside of the green space in the Kobe plot are indicated in red. The Kobe plot on the bottom shows these points in the Kobe space. The numbers in the corners indicate the percent of points in each area of the Kobe space.](../../img/PMs/PGK_med.png){width=75%}


### PGK_long

`PGK_long` calculates the probability of being in Green Zone of Kobe Space (SB>SBMSY & F<FMSY) over years 11--30 (2034--2053). This is calculated as the proportion of the total points (number of simulations x number of years) that are in the Green Zone of Kobe Space.
...
`nLRP_short` reports the probability of not breaching the LRP, calculated as $1-\text{LRP_short}$, i.e. `r 1-PMlist$LRP_short`.

![(#fig:LRPshort) An example of a time-series plot of SB/SBMSY for a two CMPs. This OM has 10 simulations, shown as the colored lines. Simulations where SB>0.4SBMSY are indicated with a green point, and red points indicate SB<0.4SBMSY. The Kobe plot on the bottom shows these points in the Kobe space. The numbers in the corners indicate the percent of points in each area of the Kobe space.](../../img/PMs/LRP_short.png){width=75%}


### LRP_long and nLRP_long

`LRP_long` calculates the probability of breaching the limit reference point (LRP; SB<0.4SBMSY) in any of years 11-30 (2034-2053).

This is calculated as the proportion of simulations where SB<0.4SBMSY in any of the last  20 years.

Figure \@ref(fig:LRPlong) shows an example of two CMPs (`r PMlist$MPs`) for an operating model with `r MSE_1@nsim` simulations. The value for `LRP_long` in this example are: `r PMlist$LRP_long`.

`nLRP_long` reports the probability of not breaching the LRP, calculated as $1-\text{LRP_long}$, i.e. `r 1-PMlist$LRP_long`.

![(#fig:LRPlong) An example of a time-series plot of SB/SBMSY for two CMPs. This OM has 10 simulations, shown as the colored lines. Simulations where SB>0.4SBMSY are indicated with a green point, and red points indicate SB<0.4SBMSY. The Kobe plot on the bottom shows these points in the Kobe space. The numbers in the corners indicate the percent of points in each area of the Kobe space.](../../img/PMs/LRP_long.png){width=75%}
...
`nLRP` reports the probability of not breaching the LRP, calculated as $1-\text{LRP}$, i.e. `r 1-PMlist$LRP`.

![(#fig:LRP) An example of a time-series plot of SB/SBMSY for a two CMPs. This OM has 10 simulations, shown as the colored lines. Simulations where SB>0.4SBMSY are indicated with a green point, and red points indicate SB<0.4SBMSY. The Kobe plot on the bottom shows these points in the Kobe space. The numbers in the corners indicate the percent of points in each area of the Kobe space.](../../img/PMs/LRP.png){width=75%}

## Yield

### TAC1

`TAC1` calculates the median TAC (t) in the first year (2024). This is calculated as the median across the simulations of the 2024 TAC (in most cases the TAC will be the same for all simulations in the first implementation year).  
...
![(#fig:C1) An example of time-series plots of the TAC for two CMPs. This OM has 10 simulations, shown as the colored lines. The points indicate the values used to calculate the median value.](../../img/PMs/C1.png){width=75%}


### AvTAC10

`AvTAC10` calculates the median landed catch (t) in the first 10 years (2024--2033). This is calculated as the median catch across the simulations and years. 

Figure \@ref(fig:AvC10) shows an example of two CMPs (`r PMlist$MPs`) for an operating model with `r MSE_1@nsim` simulations. The values for `AvTAC10` in this example are: `r round(PMlist$AvTAC10,0)`.

![(#fig:AvC10) An example of time-series plots of the TAC for a two CMPs. This OM has 10 simulations, shown as the colored lines. The points indicate the values used to calculate the median value.](../../img/PMs/AvC10.png){width=75%}

### AvTAC30

`AvTAC30` calculates the median landed catch (t) in the last 20 years (2034--2053). This is calculated as the median catch across the simulations and years. 

Figure \@ref(fig:AvC30) shows an example of two CMPs (`r PMlist$MPs`) for an operating model with `r MSE_1@nsim` simulations. The values for `AvTAC30` in this example are: `r round(PMlist$AvTAC30,0)`.

![(#fig:AvC30) An example of time-series plots of the TAC for two CMPs. This OM has 10 simulations, shown as the colored lines. The points indicate the values used to calculate the median value.](../../img/PMs/AvC30.png){width=75%}
...
Figure \@ref(fig:VarC) shows an example of two CMPs (`r PMlist$MPs`) for an operating model with `r MSE_1@nsim` simulations. The values for `VarC` in this example are: `r PMlist$VarC`. The value for `MaxVarC` in this example are: `r PMlist$MaxVarC`.

![(#fig:VarC) An example of time-series plots of the relative change in the TAC between management cycles for two CMPs. This OM has 10 simulations, shown as the colored lines. The median and maximum absolute change are shown in the top left corner.](../../img/PMs/VarC.png){width=75%}
...
```{r, echo=FALSE}
PM_tuning <- read.csv('../../dev/MP_tuning/Tuning_Target_Codes.csv')
...
There will be `r nrow(PM_tuning)` versions of each CMP corresponding to metrics and targets described in Table \@ref(tab:PM-tuning). The names of the CMPs include the tuning code (e.g., 'a', 'b', etc).

```{r PM-tuning}

caption <- 'The codes, metrics, targets, and maximum allowed change in TAC, used for the tuning of the CMPs.'
