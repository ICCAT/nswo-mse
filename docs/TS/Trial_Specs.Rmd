...
Two versions of this approach were developed. R4 uses the AMO values directly, with a period of lower than average recruitment for the first fifteen years, followed by a period of higher than average deviations (Figure \@ref(fig:r3) center panel). R4 considers the impact of just a period of negative recruitment deviations for the first fifteen years (Figure \@ref(fig:r3) right panel). 
...
## R7. Missing Data when Combined Index is updated.
...
This OM added additional multiplicative observation error to the Combined Index that was simulated in the projection years. The additional observation error was generated using the following methodology:

An analysis was conducted where the model to generate the Combined Index was re-run seven times, each time leaving out the data from the 3 most recent years for one of the fleets: Canada, Chinese-Taipei, Portugal, Spain, Japan, Morocco, and USA (Figure \@ref(fig:r7)).

```{r r7, fig.cap='The Combined Index for years 2018 -- 2022 for the Base model (all fleets) and when the data from each fleet (colored lines) were removed from the analysis. The lower plot shows the deviation from the Base model.'}
knitr::include_graphics('../../img/R7/index.png')

```

The deviation from the Base model was largest when the Spain data were removed from the analysis (Figure \@ref(fig:r7)). Log-normal observation error was generated using the standard deviation of the log deviations for the model where the Spain data were removed.

For each simulation, an integer between 1 and 4 inclusive was sampled to represent the number of times in the projection period that data were missing when the index was updated. This value was used to sample the projection years where the index was updated. The additional observation error was added to the simulated Combined Index in these years. 
