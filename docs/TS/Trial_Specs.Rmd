...
`MSEtool` is an R package that has been developed for conducting fast, flexible, and transparent, MSE for a wide range of fisheries. `MSEtool` is part of the [`openMSE`](https://cran.r-project.org/package=openMSE) collection of packages. A non-technical description of `MSEtool` and its key features is available on the [`openMSE` website](https://cran.r-project.org/package=openMSE).
