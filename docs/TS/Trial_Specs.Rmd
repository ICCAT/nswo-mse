...
For tunings *a* -- *d*, all MPs are subject to a constraint on the maximum absolute change in TAC between management cycles (Table \@ref(tab:PM-tuning)).

## Index Ratio  (IR)

### IR1

Iteratively adjusts the TAC in each management cycle $(m)$ based on the ratio of the mean Combined Index $(I)$ over the last 3 years to the mean Combined Index over the last 5 years: 

\begin{equation} 
  \text{TAC}_{m+1} = \text{TAC}_{m} \delta I_{\text{rat}} 
  (\#eq:IR1)
\end{equation} 

where $\delta$ is a tuning parameter (see Table \@ref(tab:PM-tuning)) and $I_{\text{rat}} = \frac{\bar{I}_{[m-2:m]}}{\bar{I}_{[m-4:m]}}$. 



### IR2

Same as `IR1` except that the index ratio is calculated over the last 2 and 6 years:

\begin{equation} 
 I_{\text{rat}} = \frac{\bar{I}_{[m-1:m]}}{\bar{I}_{[m-5:m]}} 
  (\#eq:IR2)
\end{equation} 
