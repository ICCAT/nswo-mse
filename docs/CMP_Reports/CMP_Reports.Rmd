...
Time-series plots and performance metric values for all CMPs that have been developed, tuned, and tested.
...
Identify the CMPs that fail to the Limit Reference Point requirement: SB does not fall below 0.4SBMSY in more than 15% of the simulations for the Reference OMs.
...
Trade-off plot identifying the CMPs that fail to achieve at least 51% probability of being in the Green Kobe space in the Medium time periods:
...
```{r}
df <- Pass_LRP %>% filter(MP %in% fail_PGK$MP)
df$Value <- round(df$Value,2)
df <- df %>% filter(PM%in%c('PGK_med', 'TAC1', 'AvTAC_med'))
df <- df %>% select(PM, MP, Value)
df <- df %>% tidyr::pivot_wider(., names_from=PM, values_from=Value)
DT::datatable(df,options = list(dom = 't', pageLength =50))
```
