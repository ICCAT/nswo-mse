---
title: "OM Summary Report"
author: "Adrian Hordyk <adrian@bluematterscience.com>"
date: "`r format(Sys.time(), '%B %d, %Y')`"
output:
  html_document:
    toc: yes
    toc_float: yes
    number_sections: yes
subtitle: 'North Atlantic Swordfish MSE '
always_allow_html: true
---


```{r setup, include=FALSE}
library(dplyr)
library(htmltools)
library(ggplot2)
library(SWOMSE)
knitr::opts_chunk$set(echo = FALSE)

fig.num <- tab.num <- 0
fig.cap <- function() {
  fig.num <<- fig.num +1
  paste0('Figure ',  fig.num, '.')

} 
tab.cap <- function() {
  tab.num <<- tab.num +1
  paste0('Table ',  tab.num, '.')
}

```

# Introduction

This report summarizes the diagnostic checks, the calculated biological reference points, and the estimated stock status relative to those reference points, for all operating models included in the OM Uncertainty Grid.  

## Operating Models 

`r tab.cap()` Summary table of the operating model uncertainty grid. `OM.num` can be used to reference the operating models in the tables and figures below. 

```{r}
DF <- OM_DF
DF <- DF %>% select(OM.num, M, sigmaR, steepness, `Include CAL`, llq, Class)
DF <- DF %>% mutate(across(1:6, as.factor))
DF$Class <- factor(DF$Class, levels=unique(DF$Class), ordered=TRUE)

DT::datatable(DF, 
              colnames=c('OM Number',
                         'M',
                         'sigmaR',
                         'Steepness (h)',
                         'Include CAL',
                         'Assumed historical increase in catchability',
                         'Class'),
              rownames= FALSE,
              filter = 'top',
              extensions = 'Buttons',
              options = list(
                pageLength = 10, 
                autoWidth = TRUE,
                sDom  = '<"top">Bflrt<"bottom">ip',
                buttons='csv')
)


```
# Model Diagnostic Checks

## Report Estimated Parameters Near Bounds

`r tab.cap()` Table of estimated parameters within 1% of the bounds.

```{r}
Check_Bounds_DF <- readRDS('Check_Bounds_DF.rda')

Check_Bounds_DF <- Check_Bounds_DF %>% mutate(across(c(1:7, 12), as.factor))
Check_Bounds_DF <- Check_Bounds_DF %>% select(OM.num, Min, Max, Value, Parameter) %>%
  relocate(OM.num, Parameter)

Check_Bounds_DF$Parameter <- factor(Check_Bounds_DF$Parameter)
DT::datatable(Check_Bounds_DF,
              rownames= FALSE,
              filter = 'top',
              extensions = 'Buttons',
              options = list(
                pageLength = 10, 
                autoWidth = TRUE,
                sDom  = '<"top">Bflrt<"bottom">ip',
                buttons='csv')
)

```

## Check for Convergence: Invertible Hessian and Final Gradient

`r tab.cap()` Table of log determinant of the Hessian matrix, (finite values indicate invertible Hessian), the maximum final gradient, high_grad (true if the final gradient for one or more parameters are > 0.0001), and the final gradient values for the estimated parameter(s).

```{r}
Check_Convergence_DF <- readRDS('Check_Convergence_DF.rda')

Check_Convergence_DF <- Check_Convergence_DF %>% select(OM.num,  log_det_hessian, invertible,
                                              max_final_gradient, 
                                              Gradient, Parameter) %>%
  relocate(OM.num, Parameter)

DT::datatable(Check_Convergence_DF,
              rownames= FALSE,
              filter = 'top',
              extensions = 'Buttons',
              options = list(
                pageLength = 10, 
                autoWidth = TRUE,
                sDom  = '<"top">Bflrt<"bottom">ip',
                buttons='csv')
)


```

## Check for Highly Correlated Parameters

`r tab.cap()` Table of operating model parameters that have correlation factor > 0.95.

```{r}
Check_Correlation_DF <- readRDS('Check_Correlation_DF.rda')

DT::datatable(Check_Correlation_DF,
              rownames= FALSE,
              filter = 'top',
              extensions = 'Buttons',
              options = list(
                pageLength = 10, 
                autoWidth = TRUE,
                sDom  = '<"top">Bflrt<"bottom">ip',
                buttons='csv')
)


```


# Summary Tables

## Biological Reference Points

`r tab.cap()` Summary table of the calculated biological reference points and estimated stock status for the OMs in the uncertainty grid. 

```{r}
ReferencePoint_DF <- readRDS('ReferencePoint_DF.rda')

RefPointDF <- ReferencePoint_DF %>% select(OM.num, SB0, SBMSY, FMSY, MSY_d, MSY_r, SB_SB0, SB_SBMSY, F_FMSY)
RefPointDF$OM.num <- factor(RefPointDF$OM.num)

RefPointDF <- RefPointDF %>% mutate(across(where(is.numeric), round, digits=2))

DT::datatable(RefPointDF,
              rownames= FALSE,
              filter = 'top',
              extensions = 'Buttons',
              options = list(
                pageLength = 10, 
                autoWidth = TRUE,
                sDom  = '<"top">Bflrt<"bottom">ip',
                buttons='csv')
)
```


# Time-Series Plots  


```{r}
imgs <- list.files(file.path('../../../../img/OM_Summary_Report/'))
Classes <- OM_DF %>% filter(Class!='Base Case') %>% select(Class)
Classes <- unique(Classes$Class)

plots <- c('SSB', 'F', 'Depletion', 'B.Bmsy', 'F.Fmsy')
headings <- c('SSB', 'Fishing mortality (F)', 'SB/SB~0~', 'SB/SB~MSY~', 'F/F~MSY~')
text <- c('spawning stock biomass (SSB)',
          'fishing mortality (F)',
          'relative spawning biomass (SB/SB0)',
          'relative spawning biomass (SB/SB_MSY)',
          'relative fishing mortality (F/F_MSY)')

```

## Reference

Time-series plots of the 9 Reference Operating Models.

```{r, results='asis', width=6, height=6}
Class <- 'Reference'
figs <- imgs[grepl(Class, imgs)]

for (i in seq_along(plots)) {
 this.fig <- figs[grepl(plots[i], figs)]
 cat('\n###', headings[i], '\n')
 cat('\n![](', file.path('../../../../img/OM_Summary_Report/', this.fig), ')')
 this.fig.num <- fig.cap()
 caption<- paste('\n\n', this.fig.num, 'Time-series of estimated', text[i], 
                     'with three levels natural mortality (M; columns) and with three levels steepness (h; colors). The black dashed line represents the predictions from the 2022 Base Case stock assessment.\n\n\n')
                     
 cat(caption)
}
```


## R1. Higher Recruitment Variability

Time-series plots of the 9 OMs where the recruitment variability was assumed to be higher than the Base Case.

```{r, results='asis', width=6, height=6}
Class <- 'R_higher_sigmaR'
figs <- imgs[grepl(Class, imgs)]

for (i in seq_along(plots)) {
 this.fig <- figs[grepl(plots[i], figs)]
 cat('\n###', headings[i], '\n')
 cat('\n![](', file.path('../../../../img/OM_Summary_Report/', this.fig), ')')
 this.fig.num <- fig.cap()
 caption<- paste('\n\n', this.fig.num, 'Time-series of estimated', text[i], 
                     'with three levels natural mortality (M; columns) and with three levels steepness (h; colors). The black dashed line represents the predictions from the 2022 Base Case stock assessment. The predictions for the Reference Operating Models are shown in colored dashed lines. \n\n\n')
                     
 cat(caption)
}
```

## R2. Remove Catch-at-Length

Time-series plots of the 9 OMs where the catch-at-length data was not included in the operating model conditioning.

```{r, results='asis', width=6, height=6}
Class <- 'R_remove_CAL'
figs <- imgs[grepl(Class, imgs)]

for (i in seq_along(plots)) {
 this.fig <- figs[grepl(plots[i], figs)]
 cat('\n###', headings[i], '\n')
 cat('\n![](', file.path('../../../../img/OM_Summary_Report/', this.fig), ')')
 this.fig.num <- fig.cap()
 caption<- paste('\n\n', this.fig.num, 'Time-series of estimated', text[i], 
                     'with three levels natural mortality (M; columns) and with three levels steepness (h; colors). The black dashed line represents the predictions from the 2022 Base Case stock assessment. The predictions for the Reference Operating Models are shown in colored dashed lines. \n\n\n')
                     
 cat(caption)
}
```


